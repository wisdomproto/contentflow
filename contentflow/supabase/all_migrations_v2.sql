...
CREATE POLICY "Users can manage own projects" ON projects FOR ALL USING (auth.uid() = user_id OR user_id IS NULL);
CREATE POLICY "Users can manage own api_keys" ON api_keys FOR ALL USING (auth.uid() = user_id OR user_id IS NULL);
CREATE POLICY "Users can manage own writing_guides" ON writing_guides FOR ALL USING (auth.uid() = user_id OR user_id IS NULL);
CREATE POLICY "Users can manage own contents" ON contents FOR ALL USING (auth.uid() = user_id OR user_id IS NULL);
